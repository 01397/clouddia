...
export const createTimeField = (value: string, className?: string, onchange: (e?: Event) => any = null): HTMLInputElement => {
  const field = h('input', { class: 'form-time ' + className, value, type: 'text' }) as HTMLInputElement;
  field.addEventListener('input', (e) => fieldInput(field, e));
  field.addEventListener('keydown', (e) => fieldKeydown(field, e));
  field.addEventListener('blur', (e) => { fieldBlur(field, e); onchange(e); });
  if (onchange !== null) field.addEventListener('change', onchange);
  return field;
};
export const createTextField = (value: string, placeholder: string = '', className?: string, onchange: (e?: Event) => any = null, oninput: (e?: Event) => any = null): HTMLInputElement => {
  const field = h('input', { class: 'form-text ' + className, value, type: 'text', placeholder }) as HTMLInputElement;
...
export const createRadio = (checked: boolean, name?: string, className?: string, onchange: (e?: Event) => any = null): HTMLInputElement => {
  const radio = h('input', { class: 'form-radio ' + className, type: 'radio', name }) as HTMLInputElement;
  radio.checked = checked;
  if (onchange !== null) radio.addEventListener('change', onchange);
  return radio;
};
export const createButton = (value: string, className?: string, onclick: (e?: Event) => any = null): HTMLInputElement => {
  return h('input', { class: 'form-button ' + className, type: 'button', value }, null, onclick) as HTMLInputElement;
};
export const createColorField = (value: string, className?: string, onchange: (e?: Event) => any = null): HTMLLabelElement => {
...
export const numberToTimeString = (number: number, format: timeFormat): string => {
  ...
  if (format === 'HH MM SS') {
    return String(Math.floor(number / 3600)).padStart(2, '0') + ' ' + String(Math.floor(number % 3600 / 60)).padStart(2, '0') + ' ' + String(number % 60).padStart(2, '0');
  }
  if (format === 'HMMSS') {
