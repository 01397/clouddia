...
export default class CanvasDiagramView extends View {
  ...
  constructor(
    ...
    const toolContainer = h('div', { id: 'dg-tools' }, [
      ...
      h('div', { id: 'dg-tools-container3', class: 'dg-tools-container' }, [
        ...
        h('label', { class: 'dg-tools-button dg-tools-button-rightMargin' }, [
          h(
            'input',
            { class: 'dg-tools-input', type: 'checkbox' },
            null,
            (e) => {
              e.stopPropagation();
    ...
    if ('ontouchstart' in document && 'orientation' in window) {
      ...
      this.element.addEventListener('click', (e) =>
        this.selectTrain(e.offsetX, e.offsetY)
      );
      this.element.addEventListener('contextmenu', (event: MouseEvent) => {
        ...
        new Menu([
          ...
          },
          {
            label: '駅時刻表で表示',
            click: () =>
              this.viewInStationTimetableView(
                trainIndex,
                direction,
                stationIndex
              ),
    ...
    if (stationId !== null && trainId !== null) {
      ...
      const time =
        this.app.data.railway.diagrams[this.selectedTrain.diaIndex].trains[
          this.selectedTrain.direction
        ][this.selectedTrain.trainIndex].timetable.data[stationId];
      // いつかは時刻のない場合でもスクロールできるようにしたいね！
      if (time.departure !== null || time.arrival !== null) {
        this.dgViewWrapper.scrollTo(
          this.getRelativeTime((time.departure || time.arrival)!) *
            this.xScale -
            this.element.offsetWidth / 2,
          this.drawingData.totalDistance[stationId] * this.yScale -
  ...
  private selectTrain(x, y) {
    ...
    if (this.selectedTrain === null) {
      ...
      this.app.sub.showStationTime({
        ...
        direction,
        train:
          this.app.data.railway.diagrams[this.diaIndex].trains[direction][
            trainIndex
          ],
      });
    }
  ...
  private scale(
    ...
    if (animation) {
      ...
      if (this.reservedScale.length !== 0) {
        [originXScale, originYScale] =
          this.reservedScale[this.reservedScale.length - 1];
      } else {
  ...
  private touchEnd(e: TouchEvent) {
    if (
      performance.now() - this.touchStartTime < 200 &&
      this.touchDragWaiting == true &&
      this.touchStartPosition
    ) {
      const { x, y } = this.touchStartPosition;
  ...
  private dragTrain() {
    ...
    if (this.dragStart && this.draggedTrain) {
      const dt =
        Math.round(
          ((x - this.dragStart.x) / this.xScale) * this.devicePixelRatio
        ) * 60;
      if (dt === 0) return;
      this.dragStart = { x, y };
      const train =
        this.app.data.railway.diagrams[this.draggedTrain.diaIndex].trains[
          this.draggedTrain.direction
  ...
  /* クリックやタップにより列車を選択 */
  // x, yはクリック座標
  private getTrainByCoordinate({ x, y }: { x: number; y: number }): {
    direction: 0 | 1;
    trainIndex: number;
    diaIndex: number;
    stationIndex: number;
    ...
    let stationIndex = this.drawingData.totalDistance.findIndex(
      (value) => ya < value
    );
    if (
      stationIndex !== 0 &&
      (this.drawingData.totalDistance[stationIndex - 1] +
        this.drawingData.totalDistance[stationIndex]) /
        2 >
        ya
    )
      stationIndex--;
  ...
  private draw() {
    ...
    if (
      ...
      this.element.style.cursor =
        this.hoveredTrain !== null ? 'pointer' : 'default';
    }
  ...
  private drawTrains(position: { x: number; y: number; w: number; h: number }) {
    ...
    if (this.selectedTrain !== null) {
      ...
      this.context.strokeStyle =
        this.app.data.railway.trainTypes[train.type].strokeColor.toHEXString();
      ...
      train.timetable.data.forEach((time, i) => {
        const stationId =
          train.direction === 0 ? i : this.stationDistance.length - i;
  ...
  private viewInTrainTimetableView(
    trainId: number,
    direction: 0 | 1,
    stationId: number
  ) {
    this.app.showTrainTimetableView(
      this.diaIndex,
      direction,
      trainId,
      stationId
