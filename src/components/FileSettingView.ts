...
export default class FileSettingView extends View {
  constructor(app: App) {
    ...
    railwayNameField.addEventListener(
      'change',
      (e) => (data.railway.name = (e.currentTarget as HTMLInputElement).value)
    );
    const railwayCommentField = h(
      'textarea',
      { class: 'form-text-multiline fs-flex' },
      data.railway.comment
    );
    railwayCommentField.addEventListener(
      'change',
      (e) =>
        (data.railway.comment = (e.currentTarget as HTMLTextAreaElement).value)
    );
    ...
    const startTimeField = createTimeField(
      ...
      null,
      (e) =>
        (data.railway.startTime = timeStringToNumber(
    ...
    const createFontRow = (font: Font, label: string) => {
      return h('div', { class: 'form-row' }, [
        ...
        createTextField(
          ...
          '書体',
          null,
          (e) => (font.family = (e.currentTarget as HTMLTextAreaElement).value)
        ),
        createTextField(
          font.height + '',
          '',
        ...
        h('label', { class: 'fs-label' }, [
          createCheckbox(
            font.bold,
            null,
            (e) => (font.bold = (e.currentTarget as HTMLInputElement).checked)
          ),
        ...
        h('label', { class: 'fs-label' }, [
          createCheckbox(
            font.italic,
            null,
            (e) => (font.italic = (e.currentTarget as HTMLInputElement).checked)
          ),
    ...
    oudiaSettings.push(
      ...data.displayProperty.timetableFont.map((font, i) =>
        createFontRow(font, '時刻表ビュー ' + (i + 1))
      ),
      createFontRow(data.displayProperty.timetableVFont, '時刻表ビュー 縦書き'),
      createFontRow(
        data.displayProperty.diagramStationFont,
        'ダイヤグラムビュー 駅名'
      ),
      createFontRow(
        data.displayProperty.diagramTimeFont,
        'ダイヤグラムビュー 時刻'
      ),
      createFontRow(
        data.displayProperty.diagramTrainFont,
        'ダイヤグラムビュー 列車'
      ),
