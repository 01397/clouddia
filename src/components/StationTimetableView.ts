  ...
  public display(changeDetail = false, trainId?: number): void {
    ...
    const clickListener = (event: MouseEvent) => {
      ...
      this.app.sub.showStationTime({
        ...
        direction: direction,
        train:
          direction == 0 ? this.inboundTrains[id] : this.outboundTrains[id],
      });
    };
    const contextmenuListener = (event: MouseEvent) => {
      ...
      new Menu([
        {
          label: '列車時刻表で表示',
          click: () => this.viewInTrainTimetableView(trainId),
        },
        {
          label: 'ダイヤグラムで表示',
          click: () => this.viewInDiagramView(trainId),
        },
    ...
    data.trains.forEach((val) => {
      ...
      const element = h(
        ...
        {
          class: 'st-train',
          'data-index': val.trainId,
          'data-direction': val.direction,
        },
        [
          h(
            'div',
