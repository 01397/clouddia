...
export default class TrainTimetableView extends View {
  ...
  constructor(
    ...
    const stationMenu: MenuItem[] = [
      {
        ...
        accelerator: 'S',
        click: () =>
          this.changeStopType(this.getActiveCell(), 1, this.direction),
      },
      {
        label: '通過',
        accelerator: 'D',
        click: () =>
          this.changeStopType(this.getActiveCell(), 2, this.direction),
      },
      {
        label: '運行なし',
        accelerator: 'F',
        click: () =>
          this.changeStopType(this.getActiveCell(), 3, this.direction),
      },
      {
        label: '時刻を消去',
    ...
    this.noTrainDialog = h(
      'div',
      { id: 'tt-noTrainButton', class: 'form-button' },
      '＋列車を追加',
      () => this.insertTrain(0)
    ) as HTMLDivElement;
    this.element.append(
    ...
    const selectByClick = (event: MouseEvent) => {
      ...
      if (!target.classList.contains('tt-cell') || !target.dataset.address)
        return;
      this.selectCell(
        ...(target.dataset.address.split('-').map((value) => Number(value)) as [
          number,
          number,
        ]),
    ...
    for (let i = 0; i < len; i++) {
      ...
      const div = h(
        'div',
        {
          ...
          style:
            `height: ${
              (Number(style.arrival[this.direction]) +
                Number(style.departure[this.direction])) *
              this.cellHeight
            }px;` +
            (stationAppearance.border && i !== len - 1
  ...
  private loadTrains() {
    ...
    this.sheet = [];
    let colIndex = 0;
    this.noTrainDialog.classList[trainList.length === 0 ? 'add' : 'remove'](
      'show'
    );
      ...
      for (let i = 0; i < stationLen; i++) {
        ...
        if (arrival) {
          ...
          if (
            ...
            departure &&
            !(i - 1 in timetable.data) &&
            i !== timetable.terminalStationIndex
          ) {
  ...
  private createColumn(colIndex: number) {
    ...
    const body = h(
      ...
      {
        class: 'tt-row',
        style: `transform: translateZ(0) translateX(${
          this.cellWidth * colIndex
        }px);top:${this.headerHeight}px;width:${this.cellWidth}px;`,
    ...
    const header = h(
      'div',
      {
        ...
        style: `transform: translateZ(0) translateX(${
          this.cellWidth * colIndex
        }px);color: ${data.type.textColor.toHEXString()};width:${
          this.cellWidth
        }px;`,
        'data-col-id': colIndex,
      },
  ...
  private reuseColumn(oldIdx: number, newIdx: number, forceUpdate: boolean) {
    ...
    Array.from(oldCol.body.children).forEach(
      (element: HTMLDivElement, i: number) => {
        ...
        if (
          oldData.cells[i].address !== newData.cells[i].address ||
          forceUpdate
        )
    ...
    oldCol.header.classList[
      this.selectedCell.some((value) => value.col === newIdx) ? 'add' : 'remove'
    ]('tt-weak-highlight');
  ...
  private changeStopType(
    cell: { col: number; stationIndex: number },
    type: number,
    direction: 0 | 1
  ) {
    const timetable =
  ...
  private moveCell(event: KeyboardEvent) {
    ...
    if (
      col < 0 ||
      row < 0 ||
      col >= this.sheet.length ||
      row >= this.sheet[0].cells.length
    )
      return;
    ...
    if (target === null) {
      ...
      if (
        ...
        this.element.scrollLeft =
          (col + 1) * this.cellWidth - this.element.offsetWidth;
      this.rendering = true;
      this.render();
  ...
  private swapTrain(from: number, to: number) {
    const trainList =
      this.app.data.railway.diagrams[this.diaIndex].trains[this.direction];
    if (
      from < 0 ||
      to < 0 ||
      from >= trainList.length ||
      to >= trainList.length
    )
      return;
  ...
  private jumpToCell({ col, stationId }) {
    ...
    this.selectCell(
      col,
      Number(target.dataset.address.split('-')[1]),
      'select'
    );
  }
  private selectCell(
    col: number,
    row: number,
    mode: 'select' | 'toggle' | 'adding' | 'deleting' = 'select'
  ) {
    const target = document.querySelector(
      `[data-address="${col}-${row}"]`
    ) as HTMLElement | null;
    ...
    const isExist = this.selectedCell.some(
      (value) => value[0] === col && value[1] === row
    );
    if (mode === 'select') {
      ...
      this.selectedCell = this.selectedCell.filter(
        (obj) =>
          obj.col !== col ||
          obj.row !== row ||
          obj.stationIndex !== stationIndex ||
          obj.cellType !== cellType
      );
    }
    this.columns.forEach(({ header }, key) => {
      header.classList[
        this.selectedCell.some((value) => value.col === key) ? 'add' : 'remove'
      ]('tt-weak-highlight');
    });
    ...
    this.focusElement.style.top =
      this.headerHeight + row * this.cellHeight - 2 + 'px';
  ...
  private viewInStationTimetableView() {
    ...
    this.app.showStationTimetableView(
      this.diaIndex,
      this.direction,
      cell.col,
      cell.stationIndex
