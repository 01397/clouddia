...
export default class TrainTimetableView extends View {
  ...
  private changeStopType(cell: { col: number; stationIndex: number }, type: number, direction: 0 | 1) {
    ...
    const stationIndex =
      direction === 0 ? cell.stationIndex : this.app.data.railway.stations.length - cell.stationIndex - 1
  ...
  private swapTrain(from: number, to: number) {
    const trainList = this.app.data.railway.diagrams[this.diaIndex].trains[this.direction]
    if (from < 0 || to < 0 || from >= trainList.length || to >= trainList.length) return;
