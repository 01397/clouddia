...
export default class TrainTimetableView extends View {
    constructor(app, diaIndex, direction) {
        ...
        this.tHeader = h('div', { id: 'tt-header', style: `height:${this.headerHeight}px` });
        ...
        this.element.addEventListener('click', (event) => {
            const trainIndex = event.target.parentElement.dataset.colId;
            const target = event.target;
            if (!('rowId' in target.dataset))
                return;
            this.focus(target);
    ...
    createColumn(colIndex) {
        ...
        const body = h('div', { class: 'tt-row', style: `transform: translateZ(0) translateX(${this.cellWidth * colIndex}px);top:${this.headerHeight}px;width:${this.cellWidth}px;`, 'data-col-id': colIndex }, bodyContent);
